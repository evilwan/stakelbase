...
//
var bail_out = false;
var en_passant_row = -1;
var en_passant_column = -1;
var can_take_ep = false;
//
// Utility function to convert BigInt values to hex string of fixed size
//
function bn2hex(bn, the_size) {
...
function parse_en_passant(str) {
    ...
    var col = str.charCodeAt(0)-97;	// asc('a') == 97
    var row = str.charCodeAt(1)-49;	// asc('1') == 49
...
//
function hash_color_to_move(arr, str){
    if(str=="w") {
	return xor_with_random(arr,780);
    } else if(str != "b") {
	bail_out = true;		
    }
    return arr;
...
function hash_castle(weed, str) {
    ...
    if(str.search("K") != -1) {
	weed = xor_with_random(weed,768);
    }
    if(str.search("Q") != -1) {
	weed = xor_with_random(weed,769);
    }
    if(str.search("k") != -1) {
	weed = xor_with_random(weed,770);
    }
    if(str.search("q") != -1) {
	weed = xor_with_random(weed,771);
    }
    return weed;
}
...
function hash_pieces(weed, str) {
	...
	for(var j = 0; j < rows[i].length; j++){
	    switch(rows[i][j]) {
                ...
                //
	    case "p":
		weed = xor_with_random(weed, 64 * 0 + 8 * row + col);					
                if((en_passant_row == 2) && (row == 3) && (en_passant_column > 0) && (col == en_passant_column - 1))
		    can_take_ep = true;
		if((en_passant_row == 2) && (row == 3) && (en_passant_column < 7) && (col == en_passant_column + 1))
		    can_take_ep = true;	
		col++;
		break;					
	    case "P":
		weed = xor_with_random(weed , 64 * 1 + 8 * row + col);
		if((en_passant_row == 5) && (row == 4) && (en_passant_column > 0) && (col == en_passant_column - 1))
		    can_take_ep = true;
		if((en_passant_row == 5) && (row == 4) && (en_passant_column < 7) && (col == en_passant_column + 1))
		    can_take_ep = true;					
		col++;
		break;
	    case "n":
		weed = xor_with_random(weed, 64 * 2 + 8 * row + col);
		col++;
		break;					
	    case "N":
		weed = xor_with_random(weed, 64 * 3 + 8 * row + col);
		col++;
		break;
	    case "b":
		weed = xor_with_random(weed, 64 * 4 + 8 * row + col);
		col++;
		break;					
	    case "B":
		weed = xor_with_random(weed, 64 * 5 + 8 * row + col);
		col++;
		break;	
	    case "r":
		weed = xor_with_random(weed, 64 * 6 + 8 * row + col);
		col++;
		break;					
	    case "R":
		weed = xor_with_random(weed, 64 * 7 + 8 * row + col);
		col++;
		break;	
	    case "q":
		weed = xor_with_random(weed, 64 * 8 + 8 * row + col);
		col++;
		break;					
	    case "Q":
		weed = xor_with_random(weed, 64 * 9 + 8 * row + col);
		col++;
		break;	
	    case "k":
		weed = xor_with_random(weed, 64 * 10 + 8 * row + col);
		col++;
		break;					
	    case "K":
		weed = xor_with_random(weed, 64 * 11 + 8 * row + col);
		col++;
		break;	
	    case "0":
	    case "1":
	    ...
	    case "7":
	    case "8":
		var r = rows[i].charCodeAt(j) - 48;	// asc('0') == 48
                col += r;
		break;
	    default:
		bail_out = true;
...
//
function compute_fen_hash(fenstring){
    en_passant_row = -1;
    en_passant_column = -1;
    can_take_ep = false;
	
    var hash = 0n;
    var fen = fenstring.trim().split(" ");
    if(fen.length!=6) {
	return null;
    }
    parse_en_passant(fen[3]); 	// will set global en-passant variables
    hash = hash_pieces(hash,fen[0]);
    if(can_take_ep) {
	hash = xor_with_random(hash, 772 + en_passant_column);
    }
    hash = hash_color_to_move(hash, fen[1]);
    hash = hash_castle(hash, fen[2]);
    return hash;
}
